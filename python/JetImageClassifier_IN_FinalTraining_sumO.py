...
mymodel = GraphNet(nParticles, len(labels), params, int(x[0]), int(x[1]), int(x[2]), 
                   int(x[3]),  int(x[4]),  int(x[5]), int(x[6]), verbose=True)
...
import h5py
f = h5py.File("%s/history_sumO.h5" %sys.argv[1], "w")
...
# the best model
torch.save(mymodel.state_dict(), "%s/IN_bestmodel_sumO.params" %(sys.argv[1]))
